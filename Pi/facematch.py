...
def main(imagePath,collectionName):
    #args = get_args()
    ...
    result, resp = check_face(client, imagePath)
    ...
    if (result):
        ...
        resu, res = check_matches(client, imagePath, collectionName)
    
        if (resu):
            return (res['FaceMatches'][0]['Face']['ExternalImageId'])
            #return (res['FaceMatches'][0]['Face']['ExternalImageId'], round(res['FaceMatches'][0]['Similarity'], 1), round(res['FaceMatches'][0]['Face']['Confidence'], 2))
            ...
            return "No face matches detected"
